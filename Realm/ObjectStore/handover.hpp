...
public:
    ...
    // Getters
    Row         row()           const { REALM_ASSERT(m_type == Type::Row);         return m_row;           }
    Query       query()         const { REALM_ASSERT(m_type == Type::Query);       return m_query;         }
    TableRef    table_ref()     const { REALM_ASSERT(m_type == Type::TableRef);    return m_table_ref;     }
    TableView   table_view()    const { REALM_ASSERT(m_type == Type::TableView);   return m_table_view;    }
    LinkViewRef link_view_ref() const { REALM_ASSERT(m_type == Type::LinkViewRef); return m_link_view_ref; }
