...
id<RLMBSON> RLMConvertBsonToRLMBSON(const Bson& b) {
    switch (b.type()) {
        ...
        case realm::bson::Bson::Type::Datetime:
            return [[NSDate alloc] initWithTimeIntervalSince1970:double(static_cast<Timestamp>(b).get_seconds()) + double(static_cast<Timestamp>(b).get_nanoseconds() / USEC_PER_SEC) / 1000];
