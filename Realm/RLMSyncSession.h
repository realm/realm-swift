...
typedef NS_ENUM(NSUInteger, RLMSyncSessionState) {
    ...
    /// with Atlas Device Sync. A session is considered Active even if
...
typedef NS_ENUM(NSUInteger, RLMSyncConnectionState) {
    ...
    /// The sync session is attempting to connect to Atlas Device Sync.
    RLMSyncConnectionStateConnecting,
    /// The sync session is currently connected to Atlas Device Sync.
...
/**
 An object encapsulating a Atlas Device Sync "session". Sessions represent the
 communication between the client (and a local Realm file on disk), and the server
 (and a remote Realm with a given partition value stored on Atlas Device Sync).
...
/**
 ...
 The session will not attempt to connect to Atlas Device Sync until `resume`
 is called or the Realm file is closed and re-opened.
 */
- (void)suspend;

/**
 Resume syncronization and reconnect to Atlas Device Sync after suspending.
