...
- (BOOL)waitForUploadsForRealm:(RLMRealm *)realm {
    ...
    BOOL couldWait = [session waitForUploadCompletionOnQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0) callback:^(NSError *) {
        dispatch_semaphore_signal(sema);
    }];
    if (!couldWait) {
        return NO;
...
- (BOOL)waitForDownloadsForRealm:(RLMRealm *)realm {
    ...
    RLMSyncSession *session = realm.syncSession;
    NSAssert(session, @"Cannot call with invalid Realm");
    BOOL couldWait = [session waitForDownloadCompletionOnQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0) callback:^(NSError *) {
        dispatch_semaphore_signal(sema);
    }];
    if (!couldWait) {
        return NO;
