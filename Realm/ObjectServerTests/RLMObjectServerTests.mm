...
- (void)testSyncErrorHandlerErrorDomain {
    ...
    RLMRealmConfiguration *c = [user createConfiguration:realmURL];
...
- (void)testOpenRealmWithAdminToken {
    ...
    RLMRealmConfiguration *c = [user createConfiguration:url];
...
- (void)testLogBackInDeferredRealmUpload {
    ...
    if (self.isParent) {
        ...
        RLMRealmConfiguration *config = [user createConfiguration:url];
        [user logOut];
...
- (void)testDownloadCancelsOnAuthError {
    RLMSyncUser *user = [self logInUserForCredentials:[RLMObjectServerTests basicCredentialsWithName:NSStringFromSelector(_cmd)
                                                                                            register:self.isParent]
                                               server:[RLMObjectServerTests authServerURL]];
    auto c = [user createConfiguration:[NSURL URLWithString:@"realm://127.0.0.1:9080/invalid"]];
...
- (void)testCompactOnLaunch {
    ...
    // actually compacted
    auto config = [user createConfiguration:url];
...
- (void)testOfflineClientReset {
    ...
    RLMSyncUser *user = [self logInUserForCredentials:[RLMObjectServerTests basicCredentialsWithName:NSStringFromSelector(_cmd)
                                                                                            register:YES]
                                               server:[RLMObjectServerTests authServerURL]];
    RLMRealmConfiguration *configuration = [user createConfiguration:REALM_URL()];
...
- (void)testPartialSync {
    ...
    @autoreleasepool {
        ...
        RLMRealmConfiguration *configuration = [user defaultConfiguration];
        RLMRealm *realm = [self openRealmWithConfiguration:configuration];
        [realm beginWriteTransaction];
