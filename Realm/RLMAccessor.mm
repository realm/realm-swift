...
static inline size_t RLMAddLinkedObject(__unsafe_unretained RLMObjectBase *link,
    ...
    else if (link.rlmRealm != realm) {
        // only try to update if link object has primary key
        if ((options & RLMCreationOptionsUpdateOrCreate) && link.rlmObjectSchema.primaryKeyProperty) {
            link = RLMCreateObjectInRealmWithValue(realm, link.rlmObjectSchema.className, link, RLMCreationOptionsUpdateOrCreate | RLMCreationOptionsAllowCopy);
        }
        else if (options & RLMCreationOptionsAllowCopy) {
            link = RLMCreateObjectInRealmWithValue(realm, link.rlmObjectSchema.className, link, RLMCreationOptionsAllowCopy);
        }
        else {
...
static inline void RLMSetValue(__unsafe_unretained RLMObjectBase *obj, NSUInteger colIndex,
    ...
    if (!val || (id)val == NSNull.null) {
        ...
        if (![[obj.rlmObjectSchema.properties[colIndex] objectClassName] isEqualToString:val.rlmObjectSchema.className]) {
            NSString *reason = [NSString stringWithFormat:@"Can't set object of type '%@' to property of type '%@'",
                                val.rlmObjectSchema.className, [obj.rlmObjectSchema.properties[colIndex] objectClassName]];
            @throw RLMException(reason);
        }

        obj->_row.set_link(colIndex, RLMAddLinkedObject(val, obj.rlmRealm, options));
    }
}
...
static inline void RLMSetValue(__unsafe_unretained RLMObjectBase *obj, NSUInteger colIndex,
    ...
    if ((id)val != NSNull.null) {
        for (RLMObjectBase *link in val) {
            linkView->add(RLMAddLinkedObject(link, obj.rlmRealm, options));
        }
    }
}
...
static inline void RLMSetValue(__unsafe_unretained RLMObjectBase *obj, NSUInteger col_ndx, __unsafe_unretained id val) {
    ...
    @throw RLMException([NSString stringWithFormat:@"Inserting invalid object of class %@ for an RLMPropertyTypeAny property (%@).", [val class], [obj.rlmObjectSchema.properties[col_ndx] name]]);
}
...
static id RLMSuperGet(RLMObjectBase *obj, NSString *propName) {
    ...
    RLMProperty *prop = obj.rlmObjectSchema[propName];
