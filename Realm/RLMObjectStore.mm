...
template <typename F>
RLMResults *RLMGetObjectsApplyingFilterToQuery(__unsafe_unretained RLMRealm *realm, __unsafe_unretained NSString *objectClassName, F applyFilter) {
    RLMCheckThread(realm);
    ...
    if (!applyFilter(&query, objectSchema)) {
