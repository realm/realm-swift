...
- (void)_fireErrorWithCode:(int)errorCode
    ...
    switch (errorClass) {
        case RLMSyncSystemErrorKindClientReset: {
            ...
            mutableUserInfo[kRLMSyncRecoveredRealmLocationPathKey] = userInfo[@(realm::SyncError::c_recovery_file_path_key)];
            std::string original_path = [userInfo[@(realm::SyncError::c_original_file_path_key)] UTF8String];
            mutableUserInfo[kRLMSyncImmediatelyRecoverRealmBlockKey] = ^{
