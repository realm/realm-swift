...
public extension SyncSession {
    ...
    struct Progress {
        ...
        let bytesDownloaded: UInt
        /// The total number of bytes that have been downloaded, and are pending download.
        /// This number represents the size of the changesets generated by all other clients
        /// using the database.
        let bytesToDownload: UInt?
        /// The number of bytes that have been uploaded.
        let bytesUploaded: UInt
        /// The total number of bytes that have been uploaded, and are pending upload.
        /// This number represents the size of the changesets representing the local changes
        /// on this client.
        let bytesToUpload: UInt?

        /**
         The fraction of bytes downloaded out of outstanding bytes to download. If the
         number of outstanding bytes to download is unknown, this property returns 0.
         */
        var fractionDownloaded: Double {
            guard let bytesToDownload = bytesToDownload else {
                return 0
            }
            let percentage = Double(bytesDownloaded)/Double(bytesToDownload)
        ...
        /**
         The fraction of bytes uploaded out of outstanding bytes to upload. If the
         number of outstanding bytes to upload is unknown, this property returns 0.
         */
        var fractionUploaded: Double {
            guard let bytesToUpload = bytesToUpload else {
                return 0
            }
            let percentage = Double(bytesUploaded)/Double(bytesToUpload)
            return percentage > 1 ? 1 : percentage
        }
        ...
        var isUploadComplete: Bool {
            ...
            return bytesUploaded >= bytesToUpload
        }

        fileprivate init(downloaded: UInt, downloadable: UInt?, uploaded: UInt, uploadable: UInt?) {
            bytesDownloaded = downloaded
            bytesToDownload = downloadable
            bytesUploaded = uploaded
            bytesToUpload = uploadable
        }
    }
    ...
    /**
     ...
     If `streaming` is true, on every invocation of the block all `Progress` values
     will reflect the latest known state. Otherwise, the `Progress` values of `bytesToDownload`
     and `bytesToUpload` will be fixed to their values when the block was registered.

     The block will be periodically called as long as the returned token is held. If `streaming`
     is false and the number of downloaded and uploaded bytes are both greater than their
     counterparts, the block will be automatically unregistered.
    ...
    func addProgressNotificationBlock(_ block: @escaping (Progress) -> Void,
                                      streaming: Bool) -> SyncSession.NotificationToken? {
        return __addProgressNotificationBlock({ (downloaded, downloadable, uploaded, uploadable) in
            block(Progress(downloaded: downloaded,
                           downloadable: downloadable as UInt?,
                           uploaded: uploaded,
                           uploadable: uploadable as UInt?))
...
public extension SyncSession {
    ...
    struct Progress {
        ...
        private init(downloaded: UInt, downloadable: UInt?, uploaded: UInt, uploadable: UInt?) {
            bytesDownloaded = downloaded
            bytesToDownload = downloadable
            bytesUploaded = uploaded
            bytesToUpload = uploadable
