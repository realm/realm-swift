...
extension RealmCollection {
    ...
    func observe(on queue: DispatchQueue?, _ block: @escaping (RealmCollectionChange<Self>) -> Void) -> NotificationToken {
        _observe(nil, queue, { change in
            switch change {
            case .initial(let collection):
                block(.initial(collection.unwrap()))
            case .update(let collection, let deletions, let insertions, let modifications):
                block(.update(collection.unwrap(), deletions: deletions, insertions: insertions, modifications: modifications))
...
extension RealmCollection where Element: ObjectBase {
    ...
    public func observe(keyPaths: [PartialKeyPath<Element>],
        ...
        return self._observe(keyPaths.map(_name(for:)), queue, { change in
            switch change {
            case .initial(let collection):
                block(.initial(collection.unwrap()))
            case .update(let collection, let deletions, let insertions, let modifications):
...
public struct AnyRealmCollection<Element: RealmCollectionValue>: RealmCollection, UntypedCollection {
    ...
    */
    ...
    internal func unwrap<C: RealmCollection>() -> C {
        if let c = self.base as? _AnyRealmCollection<C> {
            return c.base
        } else if let c = self as? C {
            return c
        }
        throwRealmException("\(Self.self) is cannot be unerased to \(C.self)")
    }
}
