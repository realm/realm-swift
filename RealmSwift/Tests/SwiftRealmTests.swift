...
class SwiftRealmTests: TestCase {
    ...
    func testRealmAddAndRemoveObjects() {
        ...
        XCTAssertEqual(Results(type: SwiftStringObject.self, realm: realm).count, Int(3), "Expecting 3 objects")
        realm.commitWrite()

        // test again after write transaction
        var results = Results(type: SwiftStringObject.self, realm: realm)
        ...
        XCTAssertEqual(Results(type: SwiftStringObject.self, realm: realm).count, Int(1), "Expecting 1 object")
        realm.commitWrite()

        results = Results(type: SwiftStringObject.self, realm: realm)
    ...
    func testRealmIsUpdatedAfterBackgroundUpdate() {
        ...
        // get object
        let results = Results(type: SwiftStringObject.self, realm: realm)
    ...
    func testRealmIgnoresProperties() {
        ...
        let results = Results(type: SwiftIgnoredPropertiesObject.self, realm: realm)
    ...
    func testUpdatingSortedArrayAfterBackgroundUpdate() {
        let realm = realmWithTestPath()
        let objs = Results(type: SwiftIntObject.self, realm: realm)
    ...
    func testRealmIsUpdatedImmediatelyAfterBackgroundUpdate() {
        ...
        // get object
        let results: Results<SwiftStringObject> = Results(realm: realm)
