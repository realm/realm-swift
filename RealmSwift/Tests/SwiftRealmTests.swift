...
class SwiftRealmTests: TestCase {
    ...
    func testRealmAddAndRemoveObjects() {
        ...
        XCTAssertEqual(realm.objects(SwiftStringObject.self).count, Int(3), "Expecting 3 objects")
        realm.commitWrite()

        // test again after write transaction
        var results = realm.objects(SwiftStringObject.self)
        ...
        XCTAssertEqual(realm.objects(SwiftStringObject.self).count, Int(1), "Expecting 1 object")
        realm.commitWrite()

        results = realm.objects(SwiftStringObject.self)
    ...
    func testRealmIgnoresProperties() {
        ...
        let results = realm.objects(SwiftIgnoredPropertiesObject.self)
    ...
    func testUpdatingSortedArrayAfterBackgroundUpdate() {
        let realm = realmWithTestPath()
        let objs = realm.objects(SwiftIntObject.self)
