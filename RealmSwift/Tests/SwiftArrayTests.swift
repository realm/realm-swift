...
class SwiftArrayTests: TestCase {
    ...
    func testFastEnumeration() {
        ...
        let results = Results(type: SwiftAggregateObject.self, realm: realm).filter("intCol < 100")
    ...
    func testArrayDescription() {
        ...
        let description: NSString = Results(type: SwiftEmployeeObject.self, realm: realm).description
    ...
    func testDeleteLinksAndObjectsInArray() {
        ...
        realm.add(company)
        company.employees.append(Results(type: SwiftEmployeeObject.self, realm: realm))
