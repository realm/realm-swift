...
class SwiftLinkTests: TestCase {
    ...
    func testBasicLink() {
        ...
        let owners = Results(type: SwiftOwnerObject.self, realm: realm)
        let dogs = Results(type: SwiftDogObject.self, realm: realm)
    ...
    func testMultipleOwnerLink() {
        ...
        XCTAssertEqual(Results(type: SwiftOwnerObject.self, realm: realm).count, Int(1), "Expecting 1 owner")
        XCTAssertEqual(Results(type: SwiftDogObject.self, realm: realm).count, Int(1), "Expecting 1 dog")
        ...
        XCTAssertEqual(Results(type: SwiftOwnerObject.self, realm: realm).count, Int(2), "Expecting 2 owners")
        XCTAssertEqual(Results(type: SwiftDogObject.self, realm: realm).count, Int(1), "Expecting 1 dog")
    }
    ...
    func testLinkRemoval() {
        ...
        let owner2 = Results(type: SwiftOwnerObject.self, realm: realm).first!
        XCTAssertNotNil(owner, "Should have 1 owner")
        XCTAssertNil(owner.dog, "Dog should be nullified when deleted")
        XCTAssertEqual(Results(type: SwiftDogObject.self, realm: realm).count, Int(0), "Expecting 0 dogs")
    }
}
