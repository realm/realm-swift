...
void ex_objc_group_intro()
{
    ...
    PeopleTable* table = [group getOrCreateTableWithName:@"people"
    ...
    /* Creates a group from an in memory buffer */
    TDBGroup* groupFromMemory = [TDBGroup groupWithBuffer:buffer withError:nil];
    PeopleTable* tableFromMemery = [groupFromMemory getOrCreateTableWithName:@"people"
                                                                asTableClass:[PeopleTable class]];
    ...
    /* Caution: Calling free(..) on the "buffer" is sometimes required to avoid leakage. However,
     the group that retrieves data from memory takes responsibilty for the memory allocation in this example. */
