...
* `RLMRealmConfiguration` now has a `openSyncedRealmOffline` property that
  allows a synchronized Realm file to be opened as a non-synchronized Realm
  (for example, the backup copies of a Realm generated by a client reset).
