...
const CGFloat kTinyThumbnailHeight = 25.0;
const CGFloat kPreviewThumbnailHeight = 200.0;

@interface RLMImage : NSObject

@property (nonatomic) NSImage *image;
@property (nonatomic) NSImage *thumbnail;
@property (nonatomic) NSImage *tinyThumbnail;
...
@implementation RLMImage

static NSImage *RLMThumbnailFromImage(NSImage *image, BOOL tiny) {
    NSSize imageSize = [image size];
    CGFloat imageAspectRatio = imageSize.width/imageSize.height;
    CGFloat thumbnailHeight = tiny ? kTinyThumbnailHeight : kPreviewThumbnailHeight;
    
    NSSize thumbnailSize = NSMakeSize(thumbnailHeight*imageAspectRatio, thumbnailHeight);
...
-(instancetype)initWithImage:(NSImage *)image
{
    ...
    if (self) {
        self.image = image;
        self.tinyThumbnail = RLMThumbnailFromImage(image, YES);
    }
...
{
    if (!_thumbnail) {
        _thumbnail = RLMThumbnailFromImage(self.image, NO);
    }
