...
begin
  ...
  unless all_available_devices.all? { |device| device['state'] == 'Shutdown' }
    raise 'There were non-Shutdown devices after 5 rounds of shutdown commands'
  end
  ...
  devices_json = `xcrun simctl list devices -j`
  raise 'xcrun failed!' unless $?.success?
  JSON.parse(devices_json)['devices'].flat_map { |_, devices| devices }.each do |device|
