...
: ${REALM_CORE_VERSION:=1.1.1} # set to "current" to always use the current build
...
case "$COMMAND" in
    ...
    "examples-ios-swift")
        ...
        xc "-workspace $workspace -scheme CollectionView -configuration $CONFIGURATION -destination 'name=iPhone 6' build ${CODESIGN_PARAMS}"
        exit 0
        ;;
