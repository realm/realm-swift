...
build_combined() {
    ...
    local destination device simulator variant=()
    case "$platform" in
        osx)
            device=macosx
            destination=
        ...
        tvos)
            device=appletvos
            destination=-appletvos
            simulator=appletvsimulator
            ;;
        catalyst)
            device=iphoneos
            destination=-maccatalyst
            variant=(-destination variant='Mac Catalyst')
            ;;
    esac
    ...
    local os_path="$build_products_path/$config${destination}/$product_name"
    local simulator_path="$build_products_path/$config-$simulator/$product_name"
    local out_path="build/$config/$platform"
    local xcframework_path="$out_path/$product.xcframework"

    # Build for each platform
    xc -scheme "$product" -configuration "$config" -sdk "$device" "${variant[@]}" build
    simulator_framework=()
    if [[ -n "$simulator" ]]; then
        xc -scheme "$product" -configuration "$config" -sdk "$simulator" build ONLY_ACTIVE_ARCH=NO
...
EOF
        ...
        (
          cd "${tempdir}"
          zip --symlinks -r "realm-swift-${version}.zip" "realm-${version}"
