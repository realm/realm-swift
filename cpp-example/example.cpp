...
void testgroup(const char *filename) {
// Create group with two tables
	Group toDisk;
	TestTableGroup& table1 = toDisk.GetTable<TestTableGroup>("test1");
	table1.Add("a",  1, true, Wed);
	table1.Add("LONG_MIN", LONG_MIN, false, Mon);
	table1.Add("LONG_MAX", LONG_MAX, true, Wed);
	table1.Add("_I64_MIN", INT64_MIN, false, Mon);
	table1.Add("_I64_MAX", INT64_MAX, true, Wed);
