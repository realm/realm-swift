...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        autoreleasepool {
            ...
            let encryptionKey = "1234567890123456789012345678901234567890123456789012345678901234".dataUsingEncoding(NSUTF8StringEncoding, allowLossyConversion: false)!
            Realm(path: Realm.defaultPath, readOnly: false,
                encryptionKey: encryptionKey, error: &error)
        ...
        autoreleasepool {
            ...
            if let realm = Realm(path: Realm.defaultPath,
                ...
                error: &error) {
                    self.log("Saved object: \((Results(type: EncryptionObject.self, realm: realm).first!).stringProp)")
            }
        }
    }
